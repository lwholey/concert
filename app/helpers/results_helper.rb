module ResultsHelper
  ...
  def generateResults ( results )
    results['events']['event'].each do |event|
      ...
      # if eventful returns more than one event it will be in hash format
      ...
      if (event['performers'] == nil)
        user.results.build( result_attr.merge( :band => event['title'] ) )
        continue
      end

      event['performers']['performer'].each do |performer|
        if (performer.class == Hash)
          user.results.build( result_attr.merge( :band => performer['name'] ) )
        elsif (performer[0] == 'name')
          user.results.build( result_attr.merge( :band => performer[1] ) )
        end
      end
  end
  ...
  def createSpotifyPlaylist(bandsArray, eventArray, dateArray, venueArray, detailsArray)
    ...
    class History
      attr_accessor :band, :spotifyBand, :track
    end

    history = History.new

    bandsArray.each do |band|
      trackCode, trackName, artistName, history = findSpotifyTrack(band, history);
      if (trackCode == nil)
        multiSplit(band).each do |band1|
          trackCode, trackName, artistName, history = findSpotifyTrack(band1, history)
          tracksString = appendStrings(tracksString, trackCode)
          history.band << band1
          history.spotifyBand << artistName
          history.track << trackName
          history.event << eventArray[i]
          history.date << dateArray[i]
          history.venue << venueArray[i]
          history.details << detailsArray[i]
        end     
      else
        tracksString = appendStrings(tracksString, trackCode)
        $bandHistory << band
  ...
  def findSpotifyTrack(bandName, history)
    trackCode = nil
    trackName = nil
    artistName = nil
    ...
    i = 0
    matchFound = false
    history.band.each do |band|
      if (history.band[i] == bandName)
        matchFound = true
        break
      end
      i += 1
    ...
    if (matchFound == false)
      ...
      artistName = history.spotifyBand[i]
      trackName = history.track[i]
    end
    return ([trackCode,trackName,artistName, history])
  end
